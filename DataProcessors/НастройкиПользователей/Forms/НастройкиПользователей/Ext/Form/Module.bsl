...
Процедура ДеревоНастроекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	НастройкиПользователейКлиент.ОткрытьОтчетИлиФорму(
...
Процедура СкопироватьНаСервере(ПользователиПриемник, ЛичныхНастроекОтчетов, Отчет)
	...
	Иначе
		...
		Если Результат.МассивНастроекБыстрогоДоступа.Количество() <> 0 Тогда
			ПользователиСлужебный.ПриСохраненииПрочихПользовательскихНастроек(
				Результат.МассивНастроекБыстрогоДоступа[0], ПользователиПриемник);
		КонецЕсли;
...
Процедура ОчиститьНаСервере(Пользователи = Неопределено, ВыбраннымПользователям = Ложь)
	...
	Если Результат.МассивНастроекБыстрогоДоступа.Количество() <> 0 Тогда
		ПользователиСлужебный.ПриУдаленииПрочихПользовательскихНастроек(
			Результат.МассивНастроекБыстрогоДоступа[0], ПользовательСсылка);
	КонецЕсли;
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = НастройкиПользователейКлиент.ФормированиеСтрокиКоличестваНастроек(КоличествоНастроек);
	...
	ПояснениеКомуОчищатьНастройки = НастройкиПользователейКлиент.ПояснениеПользователи(
...
Процедура СкопироватьНастройки()
	...
	ИначеЕсли ДеревоНастроек.ВыделенныеСтроки.Количество() = 1 Тогда
		...
		Если ДеревоНастроек.ТекущиеДанные.Тип = "ВариантЛичный" Тогда
			Предупреждение(НСтр("ru = 'Невозможно скопировать личный вариант отчета.
		...
		ИначеЕсли ДеревоНастроек.ТекущиеДанные.Тип = "НастройкаЛичная" Тогда
			Предупреждение(НСтр("ru = 'Невозможно скопировать настройку личного варианта отчета.
...
Функция ВыделенныеНастройки()
	...
	МассивНастроекБыстрогоДоступа = Новый Массив;
	...
	Для Каждого ВыделенныйЭлемент Из ВыделенныеЭлементы Цикл
		...
		Если ВыделеннаяНастройка.Тип = "ПользовательскаяНастройка" Тогда
			МассивНастроекБыстрогоДоступа.Добавить(ВыделеннаяНастройка.Ключи);
			Продолжить;
		КонецЕсли;
	...
	Возврат Новый Структура("МассивНастроек, МассивПерсональныхНастроек, МассивВариантовОтчетов, МассивНастроекБыстрогоДоступа",
			МассивНастроек, МассивПерсональныхНастроек, МассивВариантовОтчетов, МассивНастроекБыстрогоДоступа);
КонецФункции
