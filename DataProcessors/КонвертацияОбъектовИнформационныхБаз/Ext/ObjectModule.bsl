...
Перем ПолеПравилаРегистрацииОбъектов; // (кэш) ТаблицаЗначений - содержит правила регистрации объектов (правила только с видом "Фильтр разрешенных объектов" и для текущего плана обмена).
...
Процедура ВыполнитьАнализСообщенияОбмена() Экспорт
	...
	Попытка
		...
		Попытка
			...
			ПроизвестиЧтениеДанныхВРежимеАнализа(ЧтениеСообщения);
...
Процедура ПроизвестиЧтениеДанныхВРежимеАнализа(ЧтениеСообщения)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Объект" Тогда
			...
			Если ТипОбъектаСтрокой <> "КонстантыНабор" Тогда
				...
				// не отображаются пользователю в таблице информации статистики.
				// Поэтому сбор информации статистики для них не нужен.
				// Также не отображаются объекты, которые идентифицируются по полям поиска, а не по уникальному идентификатору ссылки.
				ЭтоКлассификатор = ПКО.СинхронизироватьПоИдентификатору И ПКО.ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли;
				
				Если Не ЭтоКлассификатор И ПКО.СинхронизироватьПоИдентификатору Тогда
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(176, ОписаниеОшибки(), "ПослеПолученияИнформацииОбУзлахОбмена (конвертация)");
		КонецПопытки;
...
Процедура ВыполнитьОтложеннуюЗаписьОбъектов()
	...
	// Сворачиваем таблицу по уникальным полям
	ОбъектыДляОтложеннойЗаписи().Свернуть("ОбъектСсылка, ОбъектЗаписанУспешно");
