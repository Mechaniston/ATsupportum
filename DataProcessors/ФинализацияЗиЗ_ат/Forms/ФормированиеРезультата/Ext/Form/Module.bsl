...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Заявка.Пустая() Тогда
		
		Сообщить("Ошибочные параметры открытия формы редактирования результатов!");
		Отказ = Истина;
		Возврат;
	...
	РезультатыЗаданий = Параметры.РезультатыЗаданий;
	ОткрытоИзЗаявки = НЕ ЗначениеЗаполнено(Параметры.РезультатыЗаданий);
	СтатусСтрокой2 = Параметры.СтатусСтрокой; //!!! см. проц.ОК(242)
	
	ЗаполнитьДерево(Заявка, Параметры.СписокРучныхТекстов);
...
&НаСервере
Функция   ОКНаСервере(ТекстРезультата)
	
	Дерево = РеквизитФормыВЗначение("ДеревоРезультатов");
	
	Для Каждого ЭлементСпискаРезультатовЗаданий Из РезультатыЗаданий Цикл
		
		Если ЗначениеЗаполнено(ЭлементСпискаРезультатовЗаданий.Значение) Тогда
			
			Планирование_Сервер_ат.ЗаписатьСвойстваЗадания(ЭлементСпискаРезультатовЗаданий.Значение,
				Новый Структура("РезультатВыполненияВHTML", ЭлементСпискаРезультатовЗаданий.Представление));
	...
	СтруктураВозврата.Вставить("ТекстРезультата", ТекстРезультата);
	
	Если ОткрытоИзЗаявки Тогда // Открыто из заявки - нужно сделать записи.
		...
		Запись.РезультатВыполненияВHTML = ТекстРезультата;
...
Процедура ОК(Команда)
	...
	СтруктураВозврата = ОКНаСервере(ТекстРезультата);
	СтруктураВозврата.Вставить("ДеревоРезультатов", ДеревоРезультатов); //??? а почему это в вышевызываемой функции сразу не сделать?
	//СтруктураВозврата.Вставить("СтатусСтрокой", Параметры.СтатусСтрокой); //а зачем это вообще тут? нормальным путём никак не передать?
	//!!! перестало работать! параметры в этой строке уже ничего не содержат почему-то
	// попытка указать этот параметр в настройках формы ни к чему не привела (думается, надо добавить флаг "Ключевой")
	// сделано "быстрое исправление" - сохраняем значение в ПриСозданииНаСервере в реквизит формы
	СтруктураВозврата.Вставить("СтатусСтрокой", СтатусСтрокой2);
	
	Если ПустаяСтрока(РаботаСHTML_КлиентСервер_ат.ПолучитьОбычныйТекстИзHTML(ТекстРезультата)) Тогда
		
		ПоказатьПредупреждение(, "Результат заявки не заполнен!", 5);
		
		Если ОткрытоИзЗаявки Тогда
			Возврат;
		КонецЕсли;
		
	Иначе
		
		Оповестить("ЗаписьРезультата", СтруктураВозврата, ВладелецФормы);
	...
	Если ЗначениеЗаполнено(НомерСтроки) Тогда
		...
		Если НЕ Строка = Неопределено Тогда
			...
			Иначе
				...
				ЭлементСпискаРезультатовЗаданий = РезультатыЗаданий.НайтиПоЗначению(Строка.Ссылка);
				
				Если ЭлементСпискаРезультатовЗаданий = Неопределено Тогда
					РезультатыЗаданий.Добавить(Строка.Ссылка, Элементы.Предпросмотр.Документ.Body.InnerHTML);
				Иначе
					ЭлементСпискаРезультатовЗаданий.Представление = Элементы.Предпросмотр.Документ.Body.InnerHTML;
				КонецЕсли;
...
Процедура ВывестиНаПредпросмотр()
	...
	Если НЕ ЗначениеЗаполнено(ТекущиеДанные.Ссылка) И НЕ ТекущиеДанные.ЭтоРучнойТекст Тогда
		
		Элементы.Предпросмотр.Документ.Body.InnerHTML = ТекстЗаявки;
		
	ИначеЕсли ТекущиеДанные.ЭтоРучнойТекст Тогда
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
			...
			РезультатЗадания = РезультатыЗаданий.НайтиПоЗначению(ТекущиеДанные.Ссылка);
			Если РезультатЗадания = Неопределено Тогда
				
				Свойства = Планирование_Сервер_ат.ПолучитьСвойстваЗадания(ТекущиеДанные.Ссылка);
				Элементы.Предпросмотр.Документ.Body.InnerHTML = Свойства.РезультатВыполненияВHTML;
				
			Иначе
				
				Элементы.Предпросмотр.Документ.Body.InnerHTML = РезультатЗадания.Представление;
		...
		Если Строка.Использовать Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(Строка.Ссылка) Тогда
					...
					РезультатЗадания = РезультатыЗаданий.НайтиПоЗначению(Строка.Ссылка);
					...
					Иначе
						...
						ТекстРезультата = ТекстРезультата + ?(ПустаяСтрока(ТекстРезультата), "", "<br>")
										+ РезультатЗадания.Представление;
...
&НаСервере
Процедура ЗаполнитьДерево(Заявка, СписокРучныхТекстов)
	...
	Если ЗначениеЗаполнено(Заявка.ТекстЗаявкиВHTML) Тогда
		
		ТекстЗаявки = Заявка.ТекстЗаявкиВHTML;
		РаботаСHTML_Сервер_ат.СоздатьВременныеФайлыКартинокТекста(Заявка, "ТекстЗаявки", ЭтаФорма, Истина);
	...
	Для Каждого СтрокаТаблицыСвязей Из ТаблицаСвязей Цикл
		...
		// Исключаются задания, по которым не проводились работы, кроме тех, которые сейчас закрываются.
		Если НЕ СтрокаТаблицыСвязей.ЭтоРучнойТекст
		   И СтрокаТаблицыСвязей.ФактическоеВремяВыполнения = 0
		   И РезультатыЗаданий.НайтиПоЗначению(СтрокаТаблицыСвязей.Объект) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ЭлементРезультатовЗаданий = РезультатыЗаданий.НайтиПоЗначению(СтрокаТаблицыСвязей.Объект);
				
		НоваяСтрока.Использовать = ?(ЭлементРезультатовЗаданий = Неопределено, СтрокаТаблицыСвязей.Использовать, ЭлементРезультатовЗаданий.Пометка);
	...
	Для Каждого ЭлементСпискаРучныхТекстов Из СписокРучныхТекстов Цикл
		
		Код = ЭлементСпискаРучныхТекстов.Значение;
		РучнойТекст = ЭлементСпискаРучныхТекстов.Представление;
		
		Если Число(Код) > 0 Тогда
			
			ПоискРодителя = ТаблицаСвязей.Найти(Код, "Код");
		...
		НоваяСтрока.Описание = "Ручной текст";
		НоваяСтрока.ЭтоРучнойТекст = Истина;
		НоваяСтрока.РучнойТекст = РучнойТекст;
		НоваяСтрока.Использовать = ЭлементСпискаРучныхТекстов.Пометка;
		НоваяСтрока.Код = Код;
...
Процедура УдалитьКартинкиВставленныеКопированием_Предпросмотр() Экспорт
	...
	РаботаСHTML_Клиент_ат.УдалитьКартинкиВставленныеКопированием(Элементы.Предпросмотр.Документ,
...
Процедура ИзменитьКонтестноеМенюЗаменыСловПоляHTML_Предпросмотр(СоответствиеКомандЗаменыСловам, ТолькоОчистить = Ложь)
	...
	РаботаСHTML_Сервер_ат.ИзменитьКонтестноеМенюЗаменыСловПоляHTML(ЭтаФорма,
		Элементы.Предпросмотр.КонтекстноеМеню, СоответствиеКомандЗаменыСловам, ТолькоОчистить, "ОбработчикКомандРаботыСHTML_Предпросмотр");
...
&НаКлиенте
Процедура ПредпросмотрОбычныйТекстПриИзменении(Элемент)
	
	Если ПустаяСтрока(ПредпросмотрОбычныйТекст) Тогда
		
		Ответ = Вопрос("Результат задания не может быть пустым!|Заполнить из задания?", РежимДиалогаВопрос.ДаНет, 30);
		
		Если НЕ Ответ = КодВозвратаДиалога.Нет Тогда
			
			ЗаполнитьПредпросмотрОбычныйТекст();
			
		КонецЕсли;
		
	КонецЕсли;
	
	ЭлементСпискаРезультатовЗаданий = РезультатыЗаданий.НайтиПоЗначению(Элементы.ДеревоРезультатов.ТекущиеДанные.Ссылка);
	
	Если НЕ ЭлементСпискаРезультатовЗаданий = Неопределено Тогда
		ЭлементСпискаРезультатовЗаданий.Представление = ПредпросмотрОбычныйТекст;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПредпросмотрОбычныйТекст()
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		...
		Если РезультатЗадания = Неопределено Тогда
			...
			Свойства = Планирование_Сервер_ат.ПолучитьСвойстваЗадания(ТекущиеДанные.Ссылка);
			ПредпросмотрОбычныйТекст = Свойства.РезультатВыполненияВHTML;
			
		Иначе
			
			ПредпросмотрОбычныйТекст = РезультатЗадания.Представление;
...
Процедура Удалить(Команда)
	...
	Если НЕ ТекущиеДанные = Неопределено
	   И Элементы.ДеревоРезультатов.ТекущиеДанные.ЭтоРучнойТекст Тогда
	   
		Родитель = ТекущиеДанные.ПолучитьРодителя();
		Если Родитель = Неопределено Тогда
			Родитель = ДеревоРезультатов;
		КонецЕсли;
