...
Процедура ОтобратьПоТекущемуЗначению(Параметры)
	...
	Метки_КлиентСервер_ат.УстановитьОтборСпискаПоТаблицеЗначений(Параметры.Форма.Список, ТипЗнч(ИсточникМеток), Параметры.ТипЗначенияМеток,
			Параметры.ИмяРеквизитаОтбора, МассивОтборовМеток, Параметры.ОтбиратьПоКлючу, Ложь,, Параметры.Форма.Элементы.ГруппаОтбораПоМеткам,
...
Процедура ЗагрузитьНастройку(Параметры)
	...
	ОткрытьФорму("Справочник.НастройкиМеток_ат.ФормаВыбора", ПараметрыОткрытияФормы,,,,, ОписаниеОповещенияОВыборе, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Процедура ОтключитьОтбор(Параметры)
	
	Метки_КлиентСервер_ат.УстановитьОтборСпискаПоТаблицеЗначений(Параметры.Форма.Список, Параметры.ТипЗначенияОбъекта, Параметры.ТипЗначенияМеток,
			Параметры.ИмяРеквизитаОтбора, Неопределено, Параметры.ОтбиратьПоКлючу, Ложь, Истина, Параметры.Форма.Элементы.ГруппаОтбораПоМеткам);
...
Процедура ОткрытьФормуНастройки(Параметры)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаНастройкиОтборовПоТегам_ат", Параметры, Форма,, ВариантОткрытияОкна.ОтдельноеОкно,, ОписаниеОповещенияОЗакрытииФормы);
...
Процедура УстановитьОтборыПослеНастройки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Метки_КлиентСервер_ат.УстановитьОтборСпискаПоТаблицеЗначений(Результат.Форма.Список, Результат.ТипЗначенияОбъекта, Результат.ТипЗначенияМеток,
			Результат.ИмяРеквизитаОтбора, Результат.ТаблицаЗначенийОтбора, Результат.ОтбиратьПоКлючу, Результат.ДобавлятьНулевоеЗначение,,
...
Процедура ЗагрузитьВыбраннуюНастройку(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Метки_КлиентСервер_ат.УстановитьОтборСпискаПоТаблицеЗначений(ДополнительныеПараметры.Форма.Список, ДополнительныеПараметры.ТипЗначенияОбъекта,
			ДополнительныеПараметры.ТипЗначенияМеток, ДополнительныеПараметры.ИмяРеквизитаОтбора, ВыбранныеМетки,
			ДополнительныеПараметры.ОтбиратьПоКлючу, СтруктураНастройки.ВключаяНезаполненные,, ДополнительныеПараметры.Форма.Элементы.ГруппаОтбораПоМеткам,
			Метки_ВызовСервера_ат.СформироватьПредставлениеОтбораИзТаблицы(ВыбранныеМетки, СтруктураНастройки.ВключаяНезаполненные));
...
Процедура ИзменитьИспользование(Форма, ИзменениеФлажком = Ложь, ИзменениеИзВыбранных = Ложь) Экспорт
	...
	Если Использование > Форма.МаксимальноеЗначениеИспользования
	 ИЛИ Использование = 2
	   И НЕ ТекущиеДанные.Частичная Тогда
		Использование = 0;
	КонецЕсли;
...
Процедура ОбновитьПринадлежностьВыбранныхМетокКПроектам(Форма) Экспорт
	...
	//РазвернутьДеревоМеток(Форма); //!!!!!HOTFIX (перенесён)
