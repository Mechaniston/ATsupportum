...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьЭлементыНастройкиЗаполнения(ЭтаФорма);
	...
	УстановитьДоступностьУчитыватьПраздники(ЭтаФорма);
	
	УточнитьДатуЗаполненности();
	
	ЗаполнитьДаннымиТекущегоГода(Параметры.ЗначениеКопирования);
	
	УстановитьСнятьПризнакСоответствияРезультатовШаблону(ЭтаФорма, Истина);
	
	УстановитьДоступностьРасписанияПредпраздничногоДня(ЭтаФорма);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	УстановитьСнятьПризнакСоответствияРезультатовШаблону(ЭтаФорма, Истина);
	УстановитьСнятьМодифицированностьШаблона(ЭтаФорма, Ложь);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Справочник.Календари.Форма.РасписаниеРаботы") Тогда
		...
		УстановитьСнятьПризнакСоответствияРезультатовШаблону(ЭтаФорма, Ложь);
		УстановитьСнятьМодифицированностьШаблона(ЭтаФорма, Истина);
		...
		Если КонтекстВыбора.Источник = "ШаблонЗаполненияВыбор" Тогда
			...
			СтрокаШаблона.ПредставлениеРасписания = ПредставлениеРасписанияДня(ЭтаФорма, КонтекстВыбора.НомерДня);
...
Процедура ДатаОтсчетаПриИзменении(Элемент)
	...
	Если Объект.ДатаОтсчета < Дата(1900, 1, 1) Тогда
		Объект.ДатаОтсчета = НачалоГода(ТекущаяДата());
	КонецЕсли;
...
Процедура ШаблонЗаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Календари.Форма.РасписаниеРаботы", ПараметрыФормы, ЭтаФорма);
...
Процедура ГоризонтПланированияПриИзменении(Элемент)
	...
	УточнитьЗаполненностьГрафика(ЭтаФорма);
...
Процедура КомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ОткрытьФормуРедактированияМногострочногоТекста(
		Элемент.ТекстРедактирования, Объект.Описание, Модифицированность, НСтр("ru = 'Описание'"));
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	УстановитьСнятьМодифицированностьРезультата(ЭтаФорма, Ложь);
...
Процедура ГрафикРаботыВыбор(Элемент, ВыбраннаяДата)
	...
	Если ДниГрафика.Получить(ВыбраннаяДата) = Неопределено Тогда
		// Включаем в график
		КалендарныеГрафикиКлиентСервер.ВставитьВФиксированноеСоответствие(ДниГрафика, ВыбраннаяДата, Истина);
		ДеньВключенВГрафик = Истина;
	Иначе
		// Исключаем из графика
		КалендарныеГрафикиКлиентСервер.УдалитьИзФиксированногоСоответствия(ДниГрафика, ВыбраннаяДата);
		ДеньВключенВГрафик = Ложь;
	КонецЕсли;
	
	// Фиксируем ручное изменение на дату
	КалендарныеГрафикиКлиентСервер.ВставитьВФиксированноеСоответствие(ДниИзмененные, ВыбраннаяДата, ДеньВключенВГрафик);
	
	Элементы.ГрафикРаботы.Обновить();
	
	УстановитьСнятьПризнакРучногоРедактирования(ЭтаФорма, Истина);
	УстановитьСнятьМодифицированностьРезультата(ЭтаФорма, Истина);
...
Процедура ЗаполнитьПредставлениеРасписания()
	...
	Для Каждого СтрокаШаблона Из Объект.ШаблонЗаполнения Цикл
		СтрокаШаблона.ПредставлениеРасписания = ПредставлениеРасписанияДня(ЭтаФорма, СтрокаШаблона.НомерСтроки);
	КонецЦикла;
	
	РасписаниеПредпраздничногоДня = ПредставлениеРасписанияДня(ЭтаФорма, 0);
	...
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			Форма.ЗаполненностьИнформационныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru = 'График работы заполнен до %1'"), 
		...
		Иначе											
			...
			Форма.ЗаполненностьИнформационныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru = 'График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2'"), 
													Формат(Форма.ДатаЗаполненности, "ДЛФ=D"),
...
Процедура ЗаполнитьПоШаблонуНаСервере(СохранятьРучноеРедактирование = Ложь)
	...
	Если РучноеРедактирование Тогда
		...
		Иначе
			УстановитьСнятьМодифицированностьРезультата(ЭтаФорма, Истина);
			УстановитьСнятьПризнакРучногоРедактирования(ЭтаФорма, Ложь);
		КонецЕсли;
	КонецЕсли;
	
	// Переносим результат в исходное соответствие заполнения, 
	// чтобы не «затереть» даты, не входящие в интервал заполнения
	ДниГрафикаСоответствие = КалендарныеГрафикиКлиентСервер.СоответствиеПоФиксированномуСоответствию(ДниГрафика);
...
Процедура ЗаполнитьДаннымиТекущегоГода(ЗначениеКопирования = Неопределено)
	...
	УстановитьСнятьМодифицированностьРезультата(ЭтаФорма, Ложь);
	УстановитьСнятьПризнакСоответствияРезультатовШаблону(ЭтаФорма, Не МодифицированностьШаблона);
...
Процедура ПрочитатьПризнакРучногоРедактирования(ТекущийОбъект, НомерГода)
	...
	УстановитьСнятьПризнакРучногоРедактирования(ЭтаФорма, ДниИзмененные.Количество() > 0);
