...
Процедура НастроитьОтборы(Команда)
	...
	ПараметрыМеток.Вставить("ТипЗначенияОбъекта",				Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее"));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначениеВРеквизитФормы(Метки_Сервер_ат.ПолучитьПолнуюТаблицуЗначенийОтбора("Справочник.Метки_ат"), "ТаблицаЗначенийОтбора");
	...
	УстановитьОтборУведомлений(Истина);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновилисьМетки" Тогда
		Элементы.Список.Обновить();
	КонецЕсли;
	
	Метки_КлиентСервер_ат.УстановитьОтборСпискаПоТаблицеЗначений(Список, Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее"),
...
&НаКлиенте
Процедура ПоказатьУведомления(Команда)
	
	Пометка = Элементы.ФормаПоказатьУведомления.Пометка;
	Элементы.ФормаПоказатьУведомления.Пометка = НЕ Пометка;
	УстановитьОтборУведомлений(Пометка);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтборУведомлений(Использование)
	
	УправляемыеФормы_КлиентСервер_ат.ИзменитьИспользованиеОтбораДинамическогоСписка(Список, "Скрыть уведомления", Использование);
	//УправляемыеФормы_КлиентСервер_ат.УстановитьОтборДинамическогоСписка(
	//	Список, "ТипУведомления_ат", ВидСравненияКомпоновкиДанных.НеЗаполнено, Неопределено, Использование,,, Список.Отбор);
