...
Функция   ОбязательныеРеквизитыЗаполнены()
	...
	Если Запись.Ссылка.Пустая() Тогда
		ТекстПредупреждения = "Статья затрат должна быть выбрана!";
	КонецЕсли;
...
Функция   Синхронизировать_Сервер()
	...
	СинхронизированнаяСтатьяЗатрат = ПодключениеКИБ_ат.ПолучитьВнешнююСсылкуПоНавигационнойСсылке(Подключение, Запись.НавигационнаяСсылка);
	Если СинхронизированнаяСтатьяЗатрат = Неопределено Тогда
