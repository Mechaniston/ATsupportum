...
Функция   ОбязательныеРеквизитыЗаполнены()
	...
	Если Запись.Ссылка.Пустая() Тогда
		ТекстПредупреждения = "Номенклатурная группа должна быть выбрана!";
	КонецЕсли;
...
Функция   Синхронизировать_Сервер()
	...
	СинхронизированнаяГруппаНоменклатуры = ПодключениеКИБ_ат.ПолучитьВнешнююСсылкуПоНавигационнойСсылке(Подключение, Запись.НавигационнаяСсылка);
	Если СинхронизированнаяГруппаНоменклатуры = Неопределено Тогда
